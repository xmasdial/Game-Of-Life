...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
			...
			case ID_GAMESTART:
				{
					...
					SetTimer(hWnd, 1, TIME_DELAY_MILLSEC, NULL);
				}
				break;
            case IDM_ABOUT:
	...
	case WM_LBUTTONUP:
		{
			HDC hdc = GetDC(hWnd);
			MyPoint pt(LOWORD(lParam), HIWORD(lParam));
			engine.ChangeStatus(hWnd, hdc, pt);
			ReleaseDC(hWnd, hdc);
		}
		break;
    case WM_PAINT:
        {
			...
			engine.DrawBackGround(hWnd, hdc);
            EndPaint(hWnd, &ps);
        }
        break;
    case WM_DESTROY:
